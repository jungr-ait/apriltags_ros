...
namespace apriltags_ros
{
  ...
  void AprilTagDetector::publishDetections(std::vector<AprilTags::TagDetection> const &detections,
      double const scale,
      double const fx,
      double const fy,
      double const py,
      ...
      std::set<int> &sFound_IDs)
  {
    BOOST_FOREACH(AprilTags::TagDetection detection, detections)
    {
      ...
      for(int i=0; i < 4; i++)
      {
        detection.p[i].first *= scale;
        detection.p[i].second *= scale;
      }
      detection.cxy.first *= scale;
      detection.cxy.second *= scale;

      detection.draw(cv_ptr->image, cv::Scalar(255,static_cast<int>(100*scale)%255,static_cast<int>(100*scale)%255));
  ...
  void AprilTagDetector::imageCb(const sensor_msgs::ImageConstPtr& msg, const sensor_msgs::CameraInfoConstPtr& cam_info)
  {
    ...
    publishDetections(detections, 1, fx, fy, py, px, cv_ptr, tag_detection_array, tag_pose_array, sFound_IDs);
    publishDetections(detections_half, 2, fx, fy, py, px, cv_ptr, tag_detection_array, tag_pose_array, sFound_IDs);
    publishDetections(detections_quat, 4, fx, fy, py, px, cv_ptr, tag_detection_array, tag_pose_array, sFound_IDs);
